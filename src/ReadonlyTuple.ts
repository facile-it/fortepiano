...
) => <TE>(ta: [TE, A]) => Kind3<F, FR, FE, [TE, B]>
export function traverseSnd<F extends URIS2>(
  F: A.Applicative2<F>,
): <A, FE, B>(
  f: (a: A) => Kind2<F, FE, B>,
) => <TE>(ta: [TE, A]) => Kind2<F, FE, [TE, B]>
...
export function traverseSnd<F extends URIS>(
  F: A.Applicative1<F>,
): <A, B>(f: (a: A) => Kind<F, B>) => <TE>(ta: [TE, A]) => Kind<F, [TE, B]>
