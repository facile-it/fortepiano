...
exports.cache = cache;
const _log = (method, request, log) => (url, options) => function_1.pipe(log.start(`${$Stri.uppercase(method)} ${url}`), TE.fromIO, TE.chain(() => request(url, options)), TE.chainFirstIOK(() => log.end(`${$Stri.uppercase(method)} ${url}`)), TE.orElseW((error) => function_1.pipe(log.end(`${$Stri.uppercase(method)} ${url}`), TE.fromIO, TE.chain(() => TE.left(error)))));
