...
const _log = (logKey, method, request) => (url, options) => function_1.pipe($RTE.picksIOK()(logKey, ({ log }) => log(`${$Stri.uppercase(method)} ${url}`)), RTE.chainTaskEitherK(() => request(url, options)));
const log = (logKey) => (client) => function_1.pipe(logKey || 'log', (logKey) => ({
