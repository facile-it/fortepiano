...
exports.cache = cache;
const _log = (method, request, log) => (url, options) => function_1.pipe(log.start(`${$Stri.uppercase(method)} ${url}`), TE.fromIO, TE.chain(() => request(url, options)), TE.chainFirstIOK(() => log.end(`${$Stri.uppercase(method)} ${url}`)), TE.orElseW(function_1.flow(TE.left, TE.chainFirstIOK(() => log.end(`${$Stri.uppercase(method)} ${url}`)))));
