...
exports.json = json;
const cache = (cache) => (http) => (Object.assign(Object.assign({}, http), { get: (url, options) => function_1.pipe([url, options], J.stringify, Ei.match(() => http.get(url, options), (key) => function_1.pipe(cache.get(key, exports.HttpResponseC(t.unknown)), TE.alt(() => function_1.pipe(http.get(url, options), TE.chainFirst((response) => function_1.pipe(response, cache.set(key, exports.HttpResponseC(io_ts_types_1.Json)), TE.altW(() => TE.of(undefined))))))))) }));
exports.cache = cache;
const pool = (http) => {
    const pool = new Map();
    return Object.assign(Object.assign({}, http), { get: (url, options) => {
...
const log = (logStart, logEnd = $L.void) => (http) => ({
