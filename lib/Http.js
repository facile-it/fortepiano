...
exports.cache = cache;
const _log = (method, request, log) => (url, options) => $MIO.salt(TE.MonadIO)(D.now, (salt) => function_1.pipe(log.start(`[${salt}] \r${$Stri.uppercase(method)} ${url}`), TE.fromIO, TE.chain(() => request(url, options)), TE.chainFirstIOK(() => log.end(`[${salt}] \r${$Stri.uppercase(method)} ${url}`)), TE.orElseW((error) => function_1.pipe(log.end(`[${salt}] \r${$Stri.uppercase(method)} ${url}`), TE.fromIO, TE.chain(() => TE.left(error))))));
