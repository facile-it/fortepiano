...
const log = (logger) => (cache) => ({
    get: (key, codec = t.unknown) => function_1.pipe(cache.get(key, codec), TE.chainFirstIOK(() => logger(`Item "${key}" retrieved from cache`))),
    set: (key, ttl) => function_1.flow(cache.set(key, ttl), TE.chainFirstIOK(() => logger(`Item "${key}" saved to cache`))),
    delete: (key) => function_1.pipe(cache.delete(key), TE.chainFirstIOK(() => logger(`Item "${key}" deleted from cache`))),
    clear: function_1.pipe(cache.clear, TE.chainFirstIOK(() => logger('Cache cleared'))),
});
exports.log = log;
