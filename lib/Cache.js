...
const log = (end, start = $L.void) => (cache) => ({
    get: (key, codec = t.unknown) => $R.salt(TE.MonadIO)(R.randomInt(0, Number.MAX_SAFE_INTEGER), (salt) => function_1.pipe(start(`[${salt}] \rItem "${key}" retrieved from cache`), TE.fromIO, TE.chain(() => cache.get(key, codec)), TE.chainFirstIOK(() => end(`[${salt}] \rItem "${key}" retrieved from cache`)))),
    set: (key, ttl) => (value) => $R.salt(TE.MonadIO)(R.randomInt(0, Number.MAX_SAFE_INTEGER), (salt) => function_1.pipe(start(`[${salt}] \rItem "${key}" saved to cache`), TE.fromIO, TE.chain(() => cache.set(key, ttl)(value)), TE.chainFirstIOK(() => end(`[${salt}] \rItem "${key}" saved to cache`)))),
