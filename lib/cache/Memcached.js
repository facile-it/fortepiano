...
const $memcached = (memcached, ttl = Infinity) => {
    const _memcached = function_2.memoize(memcached);
    return {
        get: (key, codec) => function_1.pipe($TE.tryCatch(() => new Promise((resolve, reject) => _memcached().get(key, (error, data) => {
        ...
        set: (key, codec, _ttl = ttl) => (value) => function_1.pipe($TE.tryCatch(() => new Promise((resolve, reject) => _memcached().set(key, codec.encode(value), _ttl / 1000, (error) => 
        // eslint-disable-next-line eqeqeq
        undefined != error ? reject(error) : resolve())), $Er.fromUnknown(Error(`Cannot write cache item "${key}"`)))),
        delete: (key) => function_1.pipe($TE.tryCatch(() => new Promise((resolve, reject) => _memcached().del(key, (error) => {
            // eslint-disable-next-line eqeqeq
            undefined != error ? reject(error) : resolve();
        })), $Er.fromUnknown(Error(`Cannot delete cache item "${key}"`)))),
        clear: function_1.pipe($TE.tryCatch(() => new Promise((resolve, reject) => _memcached().flush((error) => 
        // eslint-disable-next-line eqeqeq
