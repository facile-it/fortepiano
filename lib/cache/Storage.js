...
const storage = (_storage, name, ttl = Infinity) => ({
    get: (key, codec = t.unknown) => () => __awaiter(void 0, void 0, void 0, function* () {
        return function_1.pipe(_storage.getItem(`${undefined !== name ? `${name}_` : ''}${key}`), Ei.fromNullable(Error(`Cannot find cache item "${key}"`)), Ei.chain(J.parse), Ei.chainW(CacheItemC.decode), Ei.mapLeft($Er.fromUnknown(Error(`Cannot decode cache item "${key}"`))), Ei.filterOrElse(({ exp }) => Date.now() < exp, () => Error(`Cache item "${key}" is expired`)), Ei.map($O.get('value')), Ei.filterOrElse(codec.is, () => Error(`Cannot decode cache item "${key}" into "${codec.name}"`)));
    }),
