...
const request = (_fetch, method, url, options = {}) => TE.tryCatch(() => _fetch(function_1.pipe(options.query, O.fromNullable, O.map(RR.map((value) => value.toString())), O.map((query) => new URLSearchParams(query)), O.map((params) => params.toString()), O.filter(io_ts_types_1.NonEmptyString.is), O.match(() => url, (queryString) => `${url}?${queryString}`)), {
    ...
    return _response;
})), $Er.fromUnknown(Error(`Cannot make HTTP request "${$St.uppercase(method)} ${url}"`)));
const $fetch = (_fetch) => ({
