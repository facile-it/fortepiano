...
const $flydrive = (flydrive) => {
    ...
    return {
        read: (path, { fileSystem } = {}) => $TE.tryCatch(() => _flydrive()
            ...
            : ''}`))),
        write: (path, { fileSystem } = {}) => (buffer) => $TE.tryCatch(() => _flydrive().disk(fileSystem).put(path, buffer).then(function_1.constVoid), $E.fromUnknown(Error(`Cannot write file "${path}"${undefined !== fileSystem
            ? ` to file system "${fileSystem}"`
            : ''}`))),
        delete: (path, { fileSystem } = {}) => $TE.tryCatch(() => _flydrive()
            .disk(fileSystem)
            .delete(path)
            .then(({ wasDeleted }) => true !== wasDeleted ? Promise.reject() : Promise.resolve()), $E.fromUnknown(Error(`Cannot delete file "${path}"${undefined !== fileSystem
            ? ` from file system "${fileSystem}"`
            : ''}`))),
    };
};
