...
const log = (logStart, logEnd = $L.void) => (storage) => ({
    getStream: (path, options = {}) => function_1.pipe(storage.getStream(path, options), _log(['getting stream for', 'from'], path, options, {
        start: logStart,
        end: logEnd,
    })),
    getUrl: (path, options = {}) => function_1.pipe(storage.getUrl(path, options), _log(['getting URL for', 'from'], path, options, {
        start: logStart,
        end: logEnd,
    })),
